...
class ChatAgent:
    ...
    def chat(self, message: str, env_data: Optional[Dict] = None) -> str:
        """메인 채팅 인터페이스"""
        try:
            # 대화 기록에 사용자 메시지 추가
            self.conversation_history.append({"role": "user", "content": message})
            
            # 1. LangGraph 기반 AI 분석 및 도구 실행 (대화 히스토리 및 환경 데이터 포함)
            orchestrator_result = self.orchestrator.analyze_and_execute(message, self.conversation_history, env_data)
    ...
    def get_debug_info(self, message: str) -> Dict:
        """디버깅용 정보 반환"""
        try:
            orchestrator_result = self.orchestrator.analyze_and_execute(message, self.conversation_history)
